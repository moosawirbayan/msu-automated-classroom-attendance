    ...
    <View style={selectorStyles.row}>
      {options.map((opt) => (
        <TouchableOpacity
          ...
          style={[
            selectorStyles.chip,
            value === opt.value && selectorStyles.chipSelected,
          ]}
        ...
        >
          {opt.icon && (
            <Ionicons
              name={opt.icon}
              size={16}
              color={value === opt.value ? COLORS.white : COLORS.textSecondary}
              style={{ marginRight: 4 }}
            />
          )}
          <Text
            style={[
              ...
              value === opt.value && selectorStyles.chipTextSelected,
            ]}
          >
            {opt.label}
          </Text>
...
export default function EnrollStudentScreen({ route, navigation }) {
  ...
  const handleEnrollStudent = async () => {
    const checks = [
      [!studentData.student_id.trim(),    'Student ID is required'],
      [!studentData.first_name.trim(),    'First Name is required'],
      [!studentData.last_name.trim(),     'Last Name is required'],
      [!studentData.gender,               'Please select a gender'],
      [!studentData.year_level,           'Please select a year level'],
      [!studentData.program.trim(),       'Program is required'],
      [!studentData.mobile_number.trim(), 'Mobile Number is required'],
      [
        studentData.email.trim() &&
          !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(studentData.email.trim()),
        'Please enter a valid email address',
      ],
      [
        studentData.parent_email.trim() &&
          !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(studentData.parent_email.trim()),
    ...
    try {
      ...
      Alert.alert(
        'Error',
        error.response?.data?.message || 'Failed to enroll student. Please try again.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerSubtitle}>Add student to {classData.class_name}</Text>
      </LinearGradient>
        ...
        >
          ...
          <View style={styles.formSection}>
            ...
            <View style={styles.nameRow}>
              ...
              <View style={[styles.nameField, { flex: 1, marginLeft: 8 }]}>
                ...
                <TextInput
                  ...
                  onChangeText={set('middle_initial')}
            ...
            <TextInput
              ...
              onChangeText={set('mobile_number')}
...
const styles = StyleSheet.create({
  ...
  headerSubtitle:  { fontSize: 14, color: COLORS.white, opacity: 0.9, textAlign: 'center' },
  content:         { flex: 1, padding: 20 },
  iconContainer:   { alignItems: 'center', marginVertical: 20 },
  iconCircle:      { width: 100, height: 100, borderRadius: 50, backgroundColor: COLORS.grayLight, justifyContent: 'center', alignItems: 'center' },
  formSection:     { backgroundColor: COLORS.white, borderRadius: 16, padding: 20, marginBottom: 20, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
  label:           { fontSize: 14, fontWeight: '600', color: COLORS.textPrimary, marginBottom: 8, marginTop: 4 },
  input:           { borderWidth: 1, borderColor: COLORS.border, borderRadius: 10, padding: 14, fontSize: 15, marginBottom: 16, backgroundColor: COLORS.white, color: COLORS.textPrimary },
  nameRow:         { flexDirection: 'row' },
  nameField:       { flex: 1 },
  sectionHeader:   { fontSize: 16, fontWeight: 'bold', color: COLORS.textPrimary, marginTop: 12, marginBottom: 16 },
  helperText:      { fontSize: 12, color: COLORS.textSecondary, marginTop: -12, marginBottom: 16, fontStyle: 'italic' },
  qrInfoCard:      { backgroundColor: '#E8F5E9', borderRadius: 12, padding: 16, marginTop: 8 },
  qrInfoHeader:    { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  qrInfoTitle:     { fontSize: 15, fontWeight: '600', color: COLORS.success, marginLeft: 8 },
  qrInfoText:      { fontSize: 13, color: '#2E7D32', lineHeight: 20 },
  buttonContainer: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 40 },
  cancelButton:    { flex: 1, paddingVertical: 16, borderRadius: 12, borderWidth: 1, borderColor: COLORS.border, marginRight: 8, alignItems: 'center' },
  cancelButtonText:{ fontSize: 16, fontWeight: '600', color: COLORS.textSecondary },
  enrollButton:    { flex: 1, flexDirection: 'row', paddingVertical: 16, borderRadius: 12, backgroundColor: COLORS.primary, marginLeft: 8, alignItems: 'center', justifyContent: 'center', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 2 },
  enrollButtonDisabled: { opacity: 0.6 },
  enrollButtonText:{ fontSize: 16, fontWeight: 'bold', color: COLORS.white, marginLeft: 8 },
});
